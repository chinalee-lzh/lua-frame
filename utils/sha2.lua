...
local function get_precision(one)
   ...
   local k, n, m, prev_n = 0, one, one, one
   while true do
...
local function blake3(message, key, digest_size_in_bytes, message_flags, K, return_array)
   ...
   local final_H_in, final_block_length, chunk_by_chunk_output, result, wide_output = K, 0, 0, 0, 0
   ...
   local function partial(message_part)
      if message_part then
         ...
         if tail then
            ...
            if chunk_by_chunk_output then
               local pos, cached_block_no, cached_block = 0.0, 0, 0
